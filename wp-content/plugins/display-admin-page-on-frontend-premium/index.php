...
// Add more classes, so we can select more accurately for the "hide elements" tool
				...
				add_filter('vg_plugin_sdk/settings/' . VG_Admin_To_Frontend::$textname . '/get_setting', array( $this, 'filter_settings_page_values'), 10, 3);
			}
...
// Override the edit link from the "wp user frontend" plugin
			...
			if ( dapof_fs()->is_plan__premium_only( 'platform', true ) ) {
				require_once __DIR__ . '/inc/multitenant/index.php';
			}
...
// If we saved a page that was internal, add the internal parameter to the redirect so WPFA shows the "go back" button upon refresh.
		...
		function get_current_url($with_port = false) {
			...
			if (!empty($_SERVER['LOCALAPPDATA']) || $_SERVER['SERVER_ADDR'] === '127.0.0.1') {
				$with_port = false;
			}
			$pageURL = 'http';
...
// use get_language_attributes() if available.
		...
		function show_message_if_page_not_allowed($message, $title, $args) {
			...
			$permissions_message = VG_Admin_To_Frontend_Obj()->get_settings('wrong_permissions_message', sprintf(__('Tip from WP Frontend Admin:'
...
// We add the script without wp_enqueue_script because we want it to load in the header in this exact location
			...
			$data = apply_filters('vg_admin_to_frontend/backend/js_data', array(
				...
				'links_open_inside_iframe' => array(admin_url(), '&pagebuilder-edit', '#ff_preview', basename(admin_url()) . '/'),
