	...
	class WPFA_Options {
		...
		function render_troubleshooting_tab_content() {
			...
			<div class="troubleshooting tab-content">
				...
				<?php do_action( 'wp_frontend_admin/options_page/after_troubleshooting_content' ); ?>
			</div>
			<?php
		}
			...
			if (dapof_fs()->can_use_premium_code__premium_only()) {
				...
				if (!dapof_fs()->is_plan__premium_only('standard', true)) {
					if (is_multisite()) {
						...
						'desc' => sprintf( __('This is useful because you can create the frontend dashboard in this site once and all the users will manage their sites using this frontend dashboard. They will log in on this site and see the content from their own sites. I.e. All the users will use the same page dashboard.mysite.com/posts/ to manage the posts, but they will see the posts from their own subsites. <a href="%s" title="The list of sites is cached so you can click here if you want to select a site that doesn\'t appear in the list">Refresh this list of sites</a>', VG_Admin_To_Frontend::$textname), esc_url( wp_nonce_url( add_query_arg('wpfa_refresh_sites_list', 1), 'wpfa' ) ) ),
						);
						$fields[] = array(
...
//					'required' => array('enable_wpadmin_access_restrictions', 'equals', true)
			...
			if (dapof_fs()->can_use_premium_code__premium_only()) {
				...
				$this->sections['solutions']['fields'][] = array(
					'id' => 'stateful_urls_that_dont_require_referer',
					'type' => 'text',
					'title' => __('Front end URLs that don\'t require referrer parameter?', VG_Admin_To_Frontend::$textname),
					'desc' => __('We add the referrer parameter to the front end URLs that contain query strings, because it\'s useful for page builders that open live editors in the front end, so the "exit" buttons of the page builders link to the right dashboard page. You can add URL fragments separated wtih commas here to exclude URLs contaning those fragments.', VG_Admin_To_Frontend::$textname),
				);
				$this->sections['solutions']['fields'][] = array(
