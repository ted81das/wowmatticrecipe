<?php
if ( ! class_exists( 'FlowMattic_Updater' ) ) {
	class FlowMattic_Updater {
		public $key              = 'F0552F52D22BE819';
		private $product_id      = '1';
		private $product_base    = 'flowmattic';
		private $server_host     = FLOWMATTIC_UPDATES_SERVER . '/wp-json/licensor/';
		...
		function __construct( $plugin_base_file = '' ) {
			...
			if ( function_exists( 'add_action' ) ) {
				add_action(
					'admin_post_flowmattic_fupc',
					function() {
						$this->cleanUpdateInfo();
						$this->removeOldResponseForce();
						update_option( '_site_transient_update_plugins', '', false );
						update_option( '_site_transient_update_themes', '', false );
					...
					add_filter(
						...
						function( $links, $plugin_file ) {
							if ( $plugin_file === plugin_basename( $this->pluginFile ) ) {
								$links['update'] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=flowmattic_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						},
						10,
		...
		function handleServerRequest() {
			...
			switch ( $type ) {
				case 'rl': // remove license
					...
					update_option( '_site_transient_update_plugins', '', false );
					self::RemoveLicenseKey( FLOWMATTIC_PLUGIN_FILE, $message );
		...
		public function updateMessageCB( $data, $response ) {
			...
			if ( isset( $data->package ) && empty( $data->package ) ) {
				if ( empty( $data->update_denied_type ) ) {
					print "<br/><span style='display: block; border-top: 1px solid #ccc;padding-top: 5px; margin-top: 10px;'>Please <strong>activate your license</strong> or  <strong>renew support period</strong> to get latest version</span>";
				} elseif ( $data->update_denied_type == 'L' ) {
					print "<br/><span style='display: block; border-top: 1px solid #ccc;padding-top: 5px; margin-top: 10px;'>Please <strong>activate your license</strong> to get latest version</span>";
				...
				if ( ! is_wp_error( $response ) ) {
					...
					if ( is_object( $responseJson ) && ! empty( $responseJson->status ) && ! empty( $responseJson->data->new_version ) ) {
						...
						$responseJson->data->icons       = array(
							'svg' => FLOWMATTIC_PLUGIN_URL . 'assets/admin/img/icon.svg',
						);
		...
		private function SaveWPResponse( $response ) {
			...
			update_option( $key, $data, false ) or add_option( $key, $data );
		}
		...
		private function removeOldWPResponse() {
			$key       = $this->getKeyName();
			$isDeleted = true; //delete_option( $key );
		...
		public function removeOldResponseForce() {
			$key = $this->getKeyName();
			delete_option( $key );
		...
		public static function CheckWPPlugin( $purchase_key, $email, &$error = '', &$responseObj = null, $plugin_base_file = '' ) {
			$responseTransient = get_transient( 'flowmattic_license_response_check' );

			if ( false === $responseTransient ) {
				$obj = self::getInstance( $plugin_base_file );
				$obj->setEmailAddress( $email );
				$obj->removeOldResponseForce();
				$responseObj = null;
				return $obj->_CheckWPPlugin( $purchase_key, $error, $responseObj );
			}

			return $responseTransient;
		}
		...
		final function _CheckWPPlugin( $purchase_key, &$error = '', &$responseObj = null ) {
			...
			update_option( 'flowmattic_license_message', $error, false );

			// If an error, update the same in transient.
			if ( ! empty( $error ) ) {
				set_transient( 'flowmattic_license_response_check', $error, HOUR_IN_SECONDS * 24 );
			}

			// If an error, remove the license key to avoid pings to update server.
			// update_option( 'flowmattic_license_Key', '' );
